...
// Remplacer systématiquement vehicleImage('...') et 'https://...' par une URL Unsplash unique avec un paramètre v=itemIndex
content = content.replace(/image:\s*(vehicleImage\([^)]+\)|'https:\/\/[^']+'|"[^"]+")/g, (match) => {
  const baseImg = UNIQUE_HD_IMAGES[itemIndex % UNIQUE_HD_IMAGES.length];
  const uniqueUrl = `${baseImg}&v=web_${itemIndex}`;
  itemIndex++;
  return `image: '${uniqueUrl}'`;
});

fs.writeFileSync(filePath, content, 'utf8');
console.log(`✅ servicesData.js mis à jour : ${itemIndex} véhicules possèdent désormais des images HD 100% uniques !`);
